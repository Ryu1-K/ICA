...
figure(2)
g = semilogy(SIM.EsN0,SIM.MSE);
axis([min(SIM.EsN0) max(SIM.EsN0) 10^(-5) 10]);
hold on
grid on
set(g,'LineWidth',LW,'Marker',MK,'MarkerSize',MS,'MarkerFaceColor',MFC)
set(gca,'LineWidth',LW,'FontSize',FS,'FontName',FN,'Ytick',10.^([-5:1]),'PlotBoxAspectRatio',[1,1,1]);
lb(1) = ylabel('MSE');
lb(2) = xlabel('\it{E}\rm_s/\it{N}\rm_0 [dB]');
set(lb,'FontSize',FS,'FontName',FN);
